...
async function assertFetchTarget(url: URL, options: NormalizedWebToolOptions): Promise<void> {
  assertHttpUrl(url);
...
async function assertPublicTarget(url: URL, allowPrivateNetwork: boolean, lookup: WebDnsLookup): Promise<void> {
  assertHttpUrl(url);
...
async function defaultLookup(hostname: string): Promise<readonly WebDnsAddress[]> {
