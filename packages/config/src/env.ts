 ...
 * @returns 展开后的字符串
 */
export function expandEnvString(value: string, env: EnvSource = process.env, allowUndefined = false): string {
  // 使用正则表达式匹配环境变量，并进行替换
  return value.replace(ENV_PATTERN, (match, name: string, fallback: string | undefined) => {
    ...
    throw new ConfigError("CONFIG_ENV_MISSING", `缺失环境变量: ${name}，请在环境变量中定义该变量或提供默认值。`);
  });
}
