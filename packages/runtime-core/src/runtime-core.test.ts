...
describe("RuntimeCore", () => {
  ...
  it("lists registered commands for help without invoking the agent", async () => {
    const channel = new MockChannelAdapter();
    let runCount = 0;
    const runtime = new RuntimeCore({
      channels: new InMemoryChannelRegistry(),
    ...
    runtime.attachChannel(channel);
    await channel.emit(privateMessage("event-1", "/help"));

    expect(runCount).toBe(0);
    expect(sentText(channel.sent[0]?.message)).toBe(
      [
        "可用指令：",
        "/help - 查看所有可用指令",
        "/whoami - 查看当前身份",
